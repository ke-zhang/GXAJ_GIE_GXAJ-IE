...
'    Dim MPeakE As Single, MRunoffE As Single, MTimeE As Single, MNashC As Single
    ...
    Dim fcd() As Single, fmd() As Single, fxd() As Double, PPe() As Double
    Dim fc() As Single, fm() As Single
    Dim GRe As Double, fd As Single, fdmax As Single, ex2 As Single, R4 As Single
    ''''infiltration excess2''''''''
    Dim uu As Single, kim As Single
    Dim iff As Single, ffc As Single, sfx As Single
    ''''statistic'''''''
    Dim numrunoff As Integer, numpeak As Integer, sumNC As Single
    ''''chanliu''''''
    Dim gridrs() As Single, gridre() As Single
    With Rd
        .Open "select * from [WholeCatchPara] where YesOrNo= Yes", ConnectSys, adOpenStatic, adLockReadOnly
        If IsNull(Rd("Shortening")) Or IsNull(Rd("Time Interval(h)")) Or IsNull(Rd("Time Interval(h)")) Then
            MsgBox "请核实研究流域相关参数，程序中断！"
...
'        .Close
    ...
    ReDim gridrg(1 To Nx, 1 To Ny), gridQg2(1 To Nx, 1 To Ny), gridrs(1 To Nx, 1 To Ny), gridre(1 To Nx, 1 To Ny)
...
'                            xx = fr
                            ...
                            GS = GS * fr
                        Next m
                        If GWU + Pe - R1 - GRe < GWUM Then
                           GWU = GWU + Pe - R1 - GRe
                           GW = GWU + GWL + GWD
                        ElseIf GWU + GWL + Pe - R1 - GRe - GWUM >= GWLM Then
                            GWU = GWUM
                            GWL = GWLM
                            GWD = GW + Pe - R1 - GRe - GWU - GWL
                            If GWD > GWDM Then
                                GWD = GWDM
                            End If
                            GW = GWU + GWL + GWD
                        Else
                            GWL = GWU + GWL + Pe - R1 - GRe - GWUM
                            GWU = GWUM
                            GW = GWU + GWL + GWD
                        End If
                        If GW > GWM Then
...
'                End If
            ...
            If ENo = 3 Or ENo = 7 Then
                Open App.Path & "\Output\" & StationName & "\" & "时段产流" & "\" & StationName & "Rs" & "第" & ENo & "场" & i & ".asc" For Output As #100
                 Open App.Path & "\Output\" & StationName & "\" & "时段产流" & "\" & StationName & "Re" & "第" & ENo & "场" & i & ".asc" For Output As #101
                Print #100, "ncols      ", Ny
                Print #100, "nrows      ", Nx
                Print #100, "xllcorner  ", XllCorner
                Print #100, "yllcorner  ", YllCorner
                Print #100, "cellsize   ", DDem
                Print #100, "NODATA_value ", -9999
                Print #101, "ncols      ", Ny
                Print #101, "nrows      ", Nx
                Print #101, "xllcorner  ", XllCorner
                Print #101, "yllcorner  ", YllCorner
                Print #101, "cellsize   ", DDem
                Print #101, "NODATA_value ", -9999
                For x = 1 To Nx
                        For y = 1 To Ny
                                Print #100, gridrs(x, y);
                                Print #101, gridre(x, y);
                        Next y
                        Print #100,
                        Print #101,
                Next x
...
NoAccessError:
        ...
        With Rd
            ...
            Rd("RO Error(%)") = (SRunoff - ORunoff) / ORunoff * 100
            If ORunoff > 15 Then
                If Abs((SRunoff - ORunoff) / ORunoff * 100) <= 20 Then
                    numrunoff = numrunoff + 1
                End If
            Else
                If Abs(SRunoff - ORunoff) <= 3 Then
                    numrunoff = numrunoff + 1
                End If
            End If
            Rd("ObservedPeak") = OPeak
            Rd("SimulatedPeak") = SPeak
            Rd("Peak Error(%)") = (SPeak - OPeak) / OPeak * 100
            If Abs((SPeak - OPeak) / OPeak * 100) <= 20 Then
                numpeak = numpeak + 1
            End If
            Rd("Time Error") = SPeakTime - OPeakTime
            Rd("NC") = 1 - SNC / ONC
            sumNC = sumNC + 1 - SNC / ONC
            If ENo = NoEvents Then
                .AddNew
                Rd("RO Error(%)") = numrunoff
                Rd("Peak Error(%)") = numpeak
                Rd("NC") = sumNC / ENo
            End If
'            If Abs(Rd("Peak Error(%)")) > 20 Then
'                MPeakE = 0
